...
if __name__ == "__main__":
    ...
    N1 = 1000
    total_files = 0
    total_histogram = [0] * (1400 + 1)  # initialize
    for j in range(1, 1000):
        file_path = f"/work/users/s/a/sackau/UNC_work/time_dependent/Ns_100/Ns_100_subs_1_off/t_100/t_100_more_loci/fix_pt/dist_txt_files_t_100/{i}/output_t_100_subs_off_{(i-1)*1000+j}.txt"
        try:
            with open(file_path, 'r'):
                obj = sfs(file_path)
                loaded_data = obj.load_data()  # get the data
                loaded_data_1 = obj.load_data_1(loaded_data)  # filter subpop from the data
                if loaded_data_1.shape[0] == 1400:
                    ...
                    for k in range(1400 + 1):
                        total_histogram[k] += histogram[k]
                    total_files += 1
                    #averaged_histogram = [x / total_files for x in total_histogram]
                    averaged_histogram = [x / total_files for x in total_histogram]
                    #x_values = [i / 1400 for i in range(1400 + 1)]
                    x_values = [i for i in range(1400 + 1)]
                    output_folder = f"/work/users/s/a/sackau/UNC_work/time_dependent/Ns_100/Ns_100_subs_1_off/t_100/t_100_more_loci/fix_pt/average_data"
