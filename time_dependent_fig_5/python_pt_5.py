...
class sfs():
    ...
    def normalize_histogram(self, histogram, N1):
        ...
        normalized_histogram = [0] * (max_key + 1)
        for key, count in histogram.items():  # run the for loop for key and their freq
            # normalize between 0 and 1
            normalized_key = key / max_key
            bin_index = int(normalized_key * max_key)  # determine which bin the key will fall into
            bin_index = min(bin_index, max_key - 1)
            normalized_histogram[bin_index] += count * max_key
...
if __name__ == "__main__":
    ...
    N1 = 1000
    total_files = 0
    total_histogram = [0] * (1000 + 1)  # initialize
    for j in range(1, 1000):
        file_path = f"/work/users/s/a/sackau/UNC_work/time_dependent/Ns_100/Ns_100_subs_1_off/t_100/t_100_more_loci/fix_pt/fix_pt_0.5/dist_txt_files_t_100/{i}/output_t_100_subs_off_{(i-1)*1000+j}.txt"
        try:
            with open(file_path, 'r'):
                obj = sfs(file_path)
                loaded_data = obj.load_data()  # get the data
                loaded_data_1 = obj.load_data_1(loaded_data)  # filter subpop from the data
                if loaded_data_1.shape[0] == 1000:
                    count_ones = obj.count_ones_func(loaded_data_1)  # count # of ones at each locus
                    histogram = defaultdict(int, obj.create_histogram(count_ones))  # make the histogram (SFS) of the above
                    #normalized_histogram = obj.normalize_histogram(histogram, N1)  # do normalization of SFS
                    for k in range(1000 + 1):
                        total_histogram[k] += histogram[k]
                    total_files += 1
                    #averaged_histogram = [x / total_files for x in total_histogram]
                    averaged_histogram = [x / total_files for x in total_histogram]
                    #x_values = [i / 1000 for i in range(1000 + 1)]
                    x_values = [i for i in range(1000 + 1)]
                    output_folder = f"/work/users/s/a/sackau/UNC_work/time_dependent/Ns_100/Ns_100_subs_1_off/t_100/t_100_more_loci/fix_pt/fix_pt_0.5/average_data"
